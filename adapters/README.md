# MCP Adapter for ddraig-ssg

This directory contains the **ReScript MCP adapter** that connects ddraig-ssg to the poly-ssg-mcp hub.
...
adapters/
├── src/
│   └── DdraigAdapter.res  # ReScript adapter source
...
**ddraig-ssg uses Idris 2** as its primary implementation language.
The adapter wraps the Idris 2 runtime to provide MCP protocol access.

## Connection to Hub

This adapter exports:
- `name` - Satellite identifier
- `language` - Primary implementation language (Idris 2)
...
ALL SSG logic must remain in Idris 2.
The adapter may ONLY:
- Execute commands in the Idris 2 runtime
